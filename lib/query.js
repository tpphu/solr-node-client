...
/**
 * Allow  to highlight - Support Solr 4.x
 * @param {String} fl Highlight field
 * @param {String} tag HTML tag between highlight
 * @return {String}
 * @api public
 */
Query.prototype.hl = function(fl, tag, query)
{
	this.parameters.push('hl=true');
	this.parameters.push('hl.fl=' + fl);	
	this.parameters.push('hl.simple.pre=<' + tag + '>');
	this.parameters.push('hl.simple.post=<%2F' + tag + '>');
	if( query )
	{
		this.parameters.push('hl.q=' + encodeURIComponent(query));
	}
	return this;
};
 ...
 * @returns {Query}
 */
Query.prototype.df = function(fields)
{
	var self = this;
	var parameter = 'df=';
	if( typeof (fields) === 'string' ) {
