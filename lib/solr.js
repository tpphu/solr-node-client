...
log4js.addAppender(log4js.appenders.file(__dirname + '/trace_error.log'), 'trace_error');
var logger = log4js.getLogger('trace_error');
...
Admin.prototype.createCollection = function(options, callback) {
    if( !options )
    {
        throw new Error('The options should not be empty');
    }
    if( !options['name'] )
    {
        throw new Error('Create shard must be have collection name');
    }
...
var queryRequest = function(params,callback){
   if (params.fullPath.search(/id_table\:\(\)/i) >= 0) {
      var stack = new Error().stack;
      logger.error('----------------------------------------------');
      logger.error('id_table:()');
      logger.error(stack);
   }
   if (params.fullPath.search(/id\:\(\)/i) >= 0) {
      var stack = new Error().stack;
      logger.error('----------------------------------------------');
      logger.error('id:()');
      logger.error(stack);
   }
   if (params.fullPath.search(/id_reference\:\(\)/i) >= 0) {
      var stack = new Error().stack;
      logger.error('----------------------------------------------');
      logger.error('id_reference:()');
      logger.error(stack);
   }
   if (params.fullPath.search(/id\:\([^)]{5000,}\)/i) >= 0) {
      var stack = new Error().stack;
      logger.error('----------------------------------------------');
      logger.error('id:(5000+)');
      logger.error(stack);
   }
   if (params.fullPath.search(/hạn\:/i) >= 0) {
      var stack = new Error().stack;
      logger.error('----------------------------------------------');
      logger.error('hạn:');
      logger.error(stack);
