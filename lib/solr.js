...
{
	this._validateOptionsWhenCreateCore(options);
	var params = {};
...
Admin.prototype._validateOptionsWhenCreateCore = function(options)
{
	if( !options )
	{
	...
	for(var k in options)
	{
		if( ['name','dataDir', 'instanceDir', 'config', 'schema'].indexOf(k) === -1 )
		{
...
Client.prototype.update = function(data,callback){
   ...
   this.options.fullPath = [this.options.path,this.options.core,'update?commit='+ this.autoCommit +'&wt=json']
                              .filter(function(element){
                                 if(element) return true;
                                 return false;    
                              })
